...
import com.loopers.support.error.CoreErrorType.*
...
abstract class BaseControllerAdvice {
    private val log = LoggerFactory.getLogger(BaseControllerAdvice::class.java)
    ...
    fun handleBadRequest(e: MethodArgumentNotValidException): ResponseEntity<ApiResponse<*>> {
        ...
        return failureResponse(errorType = BAD_REQUEST, errorMessage = message)
    }
    ...
    fun handleBadRequest(e: HttpMessageNotReadableException): ResponseEntity<ApiResponse<*>> {
        ...
        return failureResponse(errorType = BAD_REQUEST, errorMessage = errorMessage)
    }
    ...
    fun handleBadRequest(e: ServerWebInputException): ResponseEntity<ApiResponse<*>> {
        ...
        return if (missingParams.isNotEmpty()) {
            failureResponse(errorType = BAD_REQUEST, errorMessage = "필수 요청 값 \'$missingParams\'가 누락되었습니다.")
        } else {
            failureResponse(errorType = BAD_REQUEST)
        }
    }

    @ExceptionHandler
    fun handleNotFound(e: NoResourceFoundException): ResponseEntity<ApiResponse<*>> {
        return failureResponse(errorType = NOT_FOUND)
    }

    @ExceptionHandler
    fun handle(e: Throwable): ResponseEntity<ApiResponse<*>> {
        log.error("Exception : {}", e.message, e)
        val errorType = INTERNAL_ERROR
